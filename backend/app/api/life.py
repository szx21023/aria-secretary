...
from app.schemas.life import LifeProfileWriteSchema, LifeReadSchema
from app.services import life as life_service

router = APIRouter(prefix="/api/life", tags=["life"])


@router.get("", response_model=LifeReadSchema)
async def get_life(db: AsyncSession = Depends(get_db)) -> LifeReadSchema:
...
@router.put("", response_model=LifeReadSchema)
async def put_life(payload: LifeProfileWriteSchema, db: AsyncSession = Depends(get_db)) -> LifeReadSchema:
