...
function freeHoursToday(todayEvents: Event[]): number {
  // 粗估：9:00–18:00 工作窗內，扣掉行程佔用後剩多少小時
  const WIN_START = 9 * 60;
  const WIN_END = 18 * 60;
  let busy = 0;
  for (const e of todayEvents) {
    const s = new Date(e.start_at);
    const start = Math.max(WIN_START, s.getHours() * 60 + s.getMinutes());
    const end = Math.min(WIN_END, start + durationMin(e.start_at, e.end_at));
